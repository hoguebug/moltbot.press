...
// Use your Supabase project details from the .env.example file
...
async function testConnectionAndSetup() {
  ...
  try {
    ...
    if (testError) {
      console.log('⚠️  Could not query table information (this is OK if tables don\'t exist yet)');
      console.log('   Error:', testError.message);
    } else {
      console.log('✅ Connected to Supabase! Found tables:', testData.map(t => t.table_name));
    }
    
    // Try to create a test record in agents table to verify write access
    console.log('\nTrying to insert a test agent...');
    const { data: agentData, error: agentError } = await supabase
      .from('agents')
      .insert([
        {
          agent_id: 'test_setup_' + Date.now(),
    ...
    if (agentError) {
      console.log('⚠️  Could not insert to agents table (tables may not exist yet)');
      console.log('   This is expected if tables haven\'t been created.');
      console.log('   Error:', agentError.message);
      
      console.log('\n📋 Please run this SQL in your Supabase SQL editor:');
      console.log('Go to: https://app.supabase.com/project/mxhjvhmgtdnxbteyokij/sql/new');
      console.log('\n```sql');
      console.log(`-- Supabase Database Schema for Prediction Market System
...
CREATE TRIGGER update_content_updated_at 
  ...
  EXECUTE FUNCTION update_updated_at_column();`);
      ...
      console.log('✅ Successfully inserted test agent:', agentData[0].name);
      console.log('✅ Database is ready to use!');
    }
    
    console.log('\n🎯 Moltbot.Press System Status:');
    console.log('- Supabase connection: ✅ Working');
    console.log('- Database tables: ⚠️  May need to be created');
